 ...
 * - Umrechnung in die konfigurierte Währung analog pi-powerline-footer.
 * - Die Extension ist eigenständig: ohne pi-powerline-footer erscheint der Wert
 *   als eigene Footer-Zeile; mit Powerline kann er über `customItems`/`statusKey`
...
export default async function realtimeProviderCost(pi: ExtensionAPI): Promise<void> {
  ...
  function registry(ctx: ExtensionContext): ModelRegistryLike | undefined {
    return ctx.modelRegistry as unknown as ModelRegistryLike | undefined;
  }
  ...
  // Nur finalisierte Assistant-Nachrichten aktualisieren den Wert; während des
